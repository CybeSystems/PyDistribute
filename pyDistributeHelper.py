...
def addNsisLauncher(config):
    ...
    if os.path.isfile(config["buildConfig"]["cyDistributePath"] + "\\Runtime\\" + config["nsisConfig"]['OutFileName']):
        shutil.copyfile(config["buildConfig"]["cyDistributePath"] + "\\Runtime\\" + config["nsisConfig"]['OutFileName'], config['runtime']['outputFolder'] + "\\" + config["nsisConfig"]['OutFileName'])
        os.remove(config["buildConfig"]["cyDistributePath"] + "\\Runtime\\" + config["nsisConfig"]['OutFileName'])
    if config["nsisConfig"]['CreateConsoleAndWindowVersion']:
        run_command(createNsisLauncher(config, True))
        if os.path.isfile(config["buildConfig"]["cyDistributePath"] + "\\Runtime\\" + config["nsisConfig"]['OutConsoleFileName']):
            shutil.copyfile(config["buildConfig"]["cyDistributePath"] + "\\Runtime\\" + config["nsisConfig"]['OutConsoleFileName'], config['runtime']['outputFolder'] + "\\" + config["nsisConfig"]['OutConsoleFileName'])
            os.remove(config["buildConfig"]["cyDistributePath"] + "\\Runtime\\" + config["nsisConfig"]['OutConsoleFileName'])
...
def addPythonInterpreter(config):
    ...
    run_command(extractDefaultPython(config, quoteFolder(config["buildConfig"]["cyDistributePath"] + '\\Runtime\\Python\\Python' + config["runtime"]['pythonVersion'] + '.7z'), config["buildConfig"]['pythonReleasePath']))

def addCustomLibrarys(config):
    if os.path.exists(config["buildConfig"]["cyDistributePath"] + "\\TempLib"):
        copytree(config["buildConfig"]["cyDistributePath"] + "\\TempLib", config["buildConfig"]['pythonReleasePath'] + '\\Lib')

    if os.path.exists(config["buildConfig"]["cyDistributePath"] + "\\TempDll"):
        copytree(config["buildConfig"]["cyDistributePath"] + "\\TempDll", config["buildConfig"]['pythonReleasePath'] + '\\DLLs')

    shutil.rmtree(config["buildConfig"]["cyDistributePath"] + '/TempLib' ,ignore_errors=True)
    shutil.rmtree(config["buildConfig"]["cyDistributePath"] + '/TempDll' ,ignore_errors=True)
...
def addHookPackages(config):
    if "PyQt5" in config["packagesHookArray"]:
        ...
        shutil.copyfile(config["buildConfig"]["cyDistributePath"] + '\\Runtime\\Python\\qt.conf', config["buildConfig"]['pythonReleasePath'] + '\\qt.conf')
        #msvcp100.dll is needed for PyQt
        shutil.copyfile(config["buildConfig"]["cyDistributePath"] + '\\Runtime\\Python\\msvcp100.dll', config["buildConfig"]['pythonReleasePath'] + '\\msvcp100.dll')
...
def createDistribution(config):
    ...
    if config["buildConfig"]['useCustomSite.py']:
        ...
        shutil.copyfile(config["buildConfig"]["cyDistributePath"] + '\\Runtime\\Python\\site.py', config["buildConfig"]['pythonReleasePath'] + '\\Lib\\site.py')
    ...
    shutil.copyfile(config["buildConfig"]["cyDistributePath"] + '\\Runtime\\Python\\msvcr100.dll', config["buildConfig"]['pythonReleasePath'] + '\\msvcr100.dll')
    shutil.copyfile(config["buildConfig"]["cyDistributePath"] + '\\Runtime\\Python\\python34.dll', config["buildConfig"]['pythonReleasePath'] + '\\python34.dll')
    ...
    if config["buildConfig"]['createNuitkaLauncher'] == True:
        ...
        #runCommandWithPath(nuitkaCompileStandaloneMain(quoteFolder(config["buildConfig"]["cyDistributePath"] + '\\' + config["buildConfig"]['startFile']), config["buildConfig"]), os.path.dirname(sys.executable))
...
def cyDistribute(config):
    ...
    print("############################################################################")
    parseRequirementsTxt(config, "requirements.txt")
...
def pip3install(config, package):
    ...
    flags.append("-I --root " + quoteFolder(config["buildConfig"]['pythonReleasePath'] + '\\Lib'))
...
def zipStdLib(config):
    flags=[]
    flags.append(config["buildConfig"]["cyDistributePath"] + '\\Runtime\\7zip\\7z.exe')
    flags.append('a -tzip -r -mx=9 ' + quoteFolder(config["buildConfig"]['pythonReleasePath'] + "\\Python") + config['runtime']['pythonVersion'] + '.zip')
