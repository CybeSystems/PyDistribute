...
def defaultConfig():
    ...
    config['release'] = {}
    config['release']['usePortableAppsStructure'] = True
    config['release']['create7zipRelease'] = True
    config['release']['createPortableApp'] = True
...
def validatePyDistribute(config):
    ...
    if not os.path.isfile(config['buildConfig']['appIcon']):
        ...
        sys.exit()
    if not os.path.isfile(os.path.dirname(config['runtime']['outputFolder']) + '\\help.html') and config['release']['usePortableAppsStructure']:
        print(os.path.dirname(config['runtime']['outputFolder']) + '\\help.html' + " not found (not a valid PortableApps Structure)")
        sys.exit()
    if not os.path.isdir(config['runtime']['outputFolder'] + '\\AppInfo') and config['release']['usePortableAppsStructure']:
        print(config['runtime']['outputFolder'] + '\\AppInfo' + " not found (not a valid PortableApps Structure)")
        sys.exit()
    if not os.path.isfile(os.path.dirname(os.path.dirname(config['runtime']['outputFolder'])) + "/PortableApps.comInstaller/PortableApps.comInstaller.exe") and config['release']['createPortableApp'] and config['release']['usePortableAppsStructure']:
...
def buildRelease(config):
    ...
    os.remove(config['runtime']['outputFolder'] + '\\' + config['nsisConfig']['OutConsoleFileName'])
...
def pip3install(config, package):
    ...
    flags.append("-I --root " + quoteFolder(os.path.dirname(config['runtime']['pythonReleasePath'])))
...
def create7zipRelease(config, releaseFolderName, outFile):
    flags=[]
